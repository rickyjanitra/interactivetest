...
function createShakePerson(app, dna){
...
const SHAKE_SPEED = 0.003;

const SHAKE_AMOUNT = 0.12;



const WRIST_SHAKE = 0.00;
...
app.ticker.add(() => {
    ...
    const shake =
    Math.sin(performance.now() * SHAKE_SPEED);
...
rightElbowPivot.rotation =
    ...
    poseRaise * POSE.RAISE.rightElbow +
    shake * 0.09;
...
rightWristPivot.rotation =
    ...
    poseRaise * POSE.RAISE.rightWrist
    - 0.20;
...
leftElbowPivot.rotation =
    ...
    poseRaise * POSE.RAISE.leftElbow +
    shake * 0.1;
...
leftWristPivot.rotation =
    ...
    poseRaise * POSE.RAISE.leftWrist;
...
if(currentAction === ACTION.NORMAL){

    polePivot.rotation =
        Math.sin(walkCycle) * POLE_SWAY;
...
let currentAction = ACTION.RAISE;
